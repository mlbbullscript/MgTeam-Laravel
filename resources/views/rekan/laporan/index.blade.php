...
@section('title', 'Laporan Keuangan Rekan')

@section('content')
<div class="space-y-6" x-data="{ modalGambarTerbuka: false, srcGambar: '' }">
    
    {{-- Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Laporan Keuangan Tim</h1>
            <p class="mt-1 text-sm text-gray-500">Transparansi kas keuangan. Seluruh rekan dapat memantau pergerakan kas masuk dan keluar secara terbuka.</p>
        </div>
        @php
        ...
        <div>
            @if($izinUpload)
                <a href="{{ route('rekan.laporan.create') }}" 
            ...
            @else
                <div class="inline-flex items-center gap-1.5 rounded-lg bg-amber-50 text-amber-700 border border-amber-200 px-4 py-2 text-xs font-semibold select-none">
                    🔒 Izin upload dinonaktifkan oleh Admin
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <form action="{{ route('rekan.laporan.index') }}" method="GET" class="flex flex-col md:flex-row md:items-end gap-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <x-stat-card 
            ...
            subtext="Pemasukan bersih operasional" 
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
            <h3 class="font-bold text-gray-800 text-sm tracking-wide">Transaksi Tim</h3>
                    ...
                    @forelse($laporan as $l)
                        <tr class="hover:bg-gray-50 transition-colors text-sm text-gray-700">
                            ...
                            <td class="px-6 py-4 font-semibold text-gray-900 whitespace-nowrap">
                                ...
                                @if($l->created_by === auth()->id())
                                    <span class="text-[10px] text-primary-600 font-bold bg-primary-50 px-1.5 py-0.5 rounded ml-1">Saya</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-1 text-xs text-gray-500">Seluruh transaksi kas tim akan terdaftar secara transparan di sini.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
