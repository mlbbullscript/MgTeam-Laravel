...
@section('title', 'Tambah Rekan Baru')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Tambah Rekan Baru</h1>
            <p class="mt-1 text-sm text-gray-500">Buat akun untuk rekan bisnis dan alokasikan koin saham kepemilikannya.</p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm p-6 sm:p-8">
        <form action="{{ route('superadmin.rekan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1.5 text-xs text-gray-500">Sisa koin tersedia saat ini: <strong>{{ number_format($sisaKoin, 2) }} koin</strong>.</p>
                    @error('coin_saham')
                        <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- Foto Profil --}}
                <div class="sm:col-span-2" x-data="{ photoName: null, photoPreview: null }">
