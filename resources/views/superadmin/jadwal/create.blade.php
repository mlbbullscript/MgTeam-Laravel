...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Tambah Jadwal Kerja Baru</h1>
            <p class="mt-1 text-sm text-gray-500">Buat penugasan baru untuk rekan kerja di hari tertentu. <strong>Maks. 2 orang per hari.</strong></p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm p-6 sm:p-8">
        <form action="{{ route('superadmin.jadwal.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <select name="assigned_to" 
                            ...
                            @foreach($rekan as $r)
                                <option value="{{ $r->id }}" {{ old('assigned_to') == $r->id ? 'selected' : '' }}>
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="mt-1">
                        <textarea name="description" 
                                  ...
                                  placeholder="Tulis instruksi pengerjaan tugas di sini secara jelas...">{{ old('description') }}</textarea>
                    </div>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                    @enderror
