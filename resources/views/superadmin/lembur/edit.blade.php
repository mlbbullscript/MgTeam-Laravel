...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Tugas Lembur</h1>
            <p class="mt-1 text-sm text-gray-500">Perbarui informasi tugas lembur atau penambahan poin lembur.</p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm p-6 sm:p-8">
        <form action="{{ route('superadmin.lembur.update', $lembur->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <textarea name="description" 
                                  ...
                                  placeholder="Tulis detail pekerjaan lembur yang harus diselesaikan rekan...">{{ old('description', $lembur->description) }}</textarea>
                    </div>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                    @enderror
                ...
                <div class="w-full sm:w-1/2">
                    ...
                    <div class="mt-1">
                        <input type="number" 
                               name="poin_kerja" 
                               id="poin_kerja" 
                               value="{{ old('poin_kerja', $lembur->poin_kerja) }}"
                               step="0.1"
                               min="0"
